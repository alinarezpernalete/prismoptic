<?php
	...
	class PDF extends FPDF{
		function Header(){
			...
			$this->Ln(10);
			$this->Cell(0,10,utf8_decode('Reporte de Compras. Pendientes'),'C');
			$this->Ln(10);
			$this->Cell(25,10,utf8_decode('Código'),1,0,'C',0);
			$this->Cell(35,10,utf8_decode('Proveedor'),1,0,'C',0);
			$this->Cell(30,10,'Fecha',1,0,'C',0);
			$this->Cell(40,10,utf8_decode('Pago'),1,1,'C',0);
		}
		
	}

	$connection = mysqli_connect('localhost', 'root', '', 'prismoptico');
	$consulta = "SELECT c.idcompra,c.cod_compra,p.nombre_proveedor,c.fecha_odc,c.pago_compra,c.estado_odc, COUNT(c.cod_compra) AS numero_pagos FROM compra c, proveedor p WHERE c.FK_idproveedor = p.idproveedor AND c.estado_odc = 0 GROUP BY cod_compra HAVING COUNT(cod_compra) > 0 ORDER BY c.fecha_odc DESC";
	...
	while($row = $resultado->fetch_assoc()){
		...
		if($row['numero_pagos'] > 1){ $pdf->Cell(40,10,utf8_decode('Varios'),1,1,'C',0); } else {
			$pdf->Cell(40,10,utf8_decode($descripcion_pago),1,1,'C',0);
		}
	}
